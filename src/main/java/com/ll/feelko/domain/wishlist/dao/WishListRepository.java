...
public interface WishListRepository extends JpaRepository<WishList, Long> {
    ...
    @Query("SELECT new com.ll.feelko.domain.wishlist.dto.WishListPageDto(w.id, w.experience.id, w.experience.title) FROM WishList w WHERE w.member.id = :memberId ORDER BY w.id DESC")
    Page<WishListPageDto> findIdAndExperienceIdAndTitleByMemberIdOrderByIdDesc(@Param("memberId") long memberId, Pageable pageable);
