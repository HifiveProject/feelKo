...
public interface ExperienceRepository extends JpaRepository<Experience,Long> {
    ...
    @Query("SELECT new com.ll.feelko.domain.member.dto.UploadedPageDto(e.id, e.imageUrl, e.title, e.price, e.startDate, e.endDate) FROM Experience e WHERE e.member.id = :memberId ORDER BY e.id DESC")
